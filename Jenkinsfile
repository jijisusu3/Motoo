pipeline {
    ...
    stages {
        ...
        stage('Deploy') {
            steps {
                sh "docker run -d TZ=Asia/Seoul --name=motoo_react_container -p 4000:4000 -p 8081:80 motoo_react"
                // sh "docker run --env-file /home/.env -d --name=motoo_fastapi_container -p 8080:8000 motoo_fastapi"
                sh "docker run -d TZ=Asia/Seoul --name=motoo_spring_container -p 8082:8080 motoo_spring"
